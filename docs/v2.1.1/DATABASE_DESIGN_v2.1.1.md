# DATABASE DESIGN DOCUMENT - Simple Calculator Web App v2.1.1
...
| 2.1.1   | 2026-06-18 | Nam       | Cập nhật v2.1.1: Mở rộng schema lịch sử cho biểu thức giải tích phức hợp và ánh xạ chỉ báo lịch sử |
...
erDiagram
    ...
    Firestore_History_Doc {
        ...
        string expression "Ví dụ: 'd/dx(x^2, 2) / d/dx(x^3, 2)' hoặc '2 + 3'"
        string result "Ví dụ: '0.3333333333' hoặc '5' hoặc 'Lỗi toán học'"
...
#### Định dạng chuẩn cho trường `expression` và `result` theo từng nhóm chức năng (Cập nhật v2.1.1):
...
| Business Rule (từ BRD v2.1.1) | Ánh xạ vào DB |
| :--- | :--- |
...
| **BR-18 (Chỉ báo Lịch sử ▲ / ▼)** | Các chỉ báo mũi tên `▲` và `▼` trên thanh trạng thái sáng lên nếu danh sách lịch sử có chứa ít nhất 1 bản ghi. Điều này ánh xạ trực tiếp tới điều kiện kiểm tra độ dài mảng cache: `calc_local_history.length >= 1`. |
